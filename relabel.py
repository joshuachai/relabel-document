...
def angiovue():
    print("angiovue")
    FOLDER_PATH = './AVANTI'  # Define the folder path containing raw files
    LABELS_XLSX = './2025_OCTA_HARMONISATION_LABELS.xlsx'  # Define the path to the Excel file with labels
    
    def remove_png_files(folder_path):
        # Function to remove all .png files in the specified folder
        png_files = glob.glob(os.path.join(folder_path, '*.png'))  # Get all .png files
        for png_file in png_files:
            os.remove(png_file)  # Delete each .png file
            print(f"{png_file} has been deleted")  # Print confirmation of deletion
        print("[INFO] All .png files removed. Only .raw files remain.")  # Inform the user that the operation is complete
    
    remove_png_files(FOLDER_PATH)  # Call the function to remove .png files
    
    # Load the Excel file into a DataFrame
    df = pd.read_excel(LABELS_XLSX)
    # Create a mapping from the DataFrame for renaming files based on specific criteria
    rename_map = {(str(row['Study participant ID']).strip(), str(row['Instrument'])[0].strip(), str(row['Retinal Layer'])[0].strip(), str(row['Image size [mm]'])[0].strip()): f"{int(row['Image ID']):04d}" for _, row in df.iterrows() if pd.notna(row['Image ID'])}
    
    def parse_raw_filename(filename_base):
        # Function to parse the raw filename into its components
        parts = filename_base.split('_')  # Split the filename by underscore
        if len(parts) < 2 or len(parts[1]) < 3:
            return '', '', '', ''  # Return empty strings if the filename format is incorrect
        return parts[0], parts[1][0], parts[1][1], parts[1][2]  # Return the parsed components
    
    raw_files = glob.glob(os.path.join(FOLDER_PATH, '*.raw'))  # Get all .raw files in the folder
    for raw_file in raw_files:
        base_name = os.path.splitext(os.path.basename(raw_file))[0]  # Get the base name of the raw file
        key = parse_raw_filename(base_name)  # Parse the filename to create a key for renaming
        
        if key in rename_map:  # Check if the key exists in the rename map
            new_name = f"{rename_map[key]}.raw"  # Create the new filename based on the mapping
            os.rename(raw_file, os.path.join(FOLDER_PATH, new_name))  # Rename the raw file
...
def extractFileName():
    ...
    if (sys.argv[1] == "revo"):
        ...
        # If the argument is 'angiovue', it will excute the function named 'angiovue'
    elif sys.argv[1] == "angiovue":
        angiovue()
